...
export class PinchZoomComponent implements OnDestroy {
  ...
  ngOnChanges(changes: { [x: string]: { currentValue: {}; }; }) {
  ...
  getProperties(changes: { [x: string]: { currentValue: {}; }; }) {
  ...
  renameProperties(options: Properties) {
    for (var prop in options) {
      if (backwardCompatibilityProperties[prop]) {
        options[backwardCompatibilityProperties[prop]] = options[prop] as Properties;
        delete options[prop];
      }
    }
